...
#Compilamos el modelo de la red neuronal (Optimizador SGD)
...
with open(f"{path}model_sgd.json", "w") as json_file:
    json_file.write(model_json)
# Serializamos los pesos a HDF5
model.save_weights(f"{path}model_sgd.h5")
print("Saved model to disk")

# Cargamos el JSON y creamos el modelo a partir del JSON
json_file = open(f"{path}model_sgd.json", "r")
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)

# Cargamos los pesos en el nuevo modelo
loaded_model.load_weights(f"{path}model_sgd.h5")
...
axs[0].set_title(f'Precision de entrenamiento - Optimizador SGD')
axs[0].legend()

axs[1].plot(epochs, loss, 'b', label='Training loss')
axs[1].set_title(f'Suma de las perdidas - Optimizador SGD')
axs[1].legend()
